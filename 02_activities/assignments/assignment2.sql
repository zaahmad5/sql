...
-- Step 1: Aggregate sales grouped by market date.
WITH SalesByDate AS (
    SELECT market_date,
...
-- Step 3: Retrieve the best and worst days using UNION.
SELECT market_date,
    total_sales,
...
SELECT vp.vendor_name,
    vp.product_name,
    vp.original_price * 5 * cc.total_customers AS total_revenue_per_product
FROM VendorProducts vp
CROSS JOIN CustomerCount cc
GROUP BY vp.vendor_name, vp.product_name, vp.original_price, cc.total_customers;
...
SET current_quantity = (
    ...
    ON vi.product_id = latest_dates.product_id
    AND vi.market_date = latest_dates.last_market_date
